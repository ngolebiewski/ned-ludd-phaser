...
export class BossBattle extends Scene {
  ...
  create() {
    ...
    // 1. MUSIC
    this.sound.stopAll();
    this.bossMusic = this.sound.add("factory_song");
    this.bossMusic.play({ loop: true, volume: 0.4, rate: 1.5 });
    ...
    for (let x = 0; x < width; x += TILE_SIZE) {
      for (let y = 0; y < height; y += TILE_SIZE) {
        ...
        const g = 110 + v;
        const bgTile = this.add.image(x + 32, y + 32, "factory", TILES.BG_TILE)
          .setOrigin(0.5).setDepth(0).setAlpha(0.9).setTint(Display.Color.GetColor(g, g, g));
    ...
    this.physics.add.overlap(this.player, this.boss, () => {
      ...
      } else if (!this.player.isSmashing && !this.isInvulnerable && this.boss.isActive) {
  ...
  showTerminalText(content, onComplete) {
    ...
    const label = this.add.text(width / 2, height / 2, "", {
      fontFamily: "Departure Mono", fontSize: "22px", color: "#00ff00", align: "center", stroke: "#000", strokeThickness: 4
  ...
  handlePlayerHit(isLaser = false) {
    ...
    if (this.playerHP <= 0) this.handleGameOver();
      ...
      this.tweens.add({ targets: this.player, alpha: 0.5, yoyo: true, repeat: 5, duration: 100, onComplete: () => { this.player.alpha = 1; this.isInvulnerable = false; } });
    }
  }
  ...
  handleGameOver() {
    ...
    this.bossMusic.stop();
    const { width, height } = this.scale;
    const goText = this.add.text(width / 2, height / 2, "GAME OVER", { fontFamily: "Departure Mono", fontSize: "80px", color: "#ff0000", stroke: "#000", strokeThickness: 8 }).setOrigin(0.5).setDepth(200).setScale(0);
    this.tweens.add({ targets: goText, scale: 1, duration: 500, ease: "Back.easeOut", onComplete: () => { this.time.delayedCall(3000, () => this.scene.start("Title")); } });
  }
  ...
  handleWin() {
    ...
    this.time.delayedCall(1000, () => {
      this.showTerminalText("YOU ARE VICTORIOUS TODAY,\nBUT THE MACHINES ARE UNSTOPPABLE", () => { this.time.delayedCall(2000, () => this.scene.start("Title")); });
    });
  }
